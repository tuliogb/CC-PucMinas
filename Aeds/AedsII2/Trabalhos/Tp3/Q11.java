...
public class CtrlPersonagem{
    ...
    public CtrlPersonagem prox, ant;

    static int tamBase=0, maxTam=405, comparacoes=0, movimentacoes=0;
    ...
    static void swap(CtrlPersonagem x, CtrlPersonagem y) {
    ...
    static void desempataNome(CtrlPersonagem esq, CtrlPersonagem dir) {
        CtrlPersonagem i = esq, j = dir;
        Personagem pivo = mediaPersonagem(esq, dir);
        ...
        while(j.prox!=i && j.prox!=i.ant){
            ...
            if(j.prox!=i && j.prox!=i.ant){
                if(i.elemento.getHouse().equals(j.elemento.getHouse()) && i.elemento.getName().compareTo(j.elemento.getName()) > 0){
                    swap(i, j);
                    i=i.prox;
                    j=j.ant;
                }else{
    ...
    static boolean compareHouse(Personagem pivo, CtrlPersonagem temp, int selecao) {
        boolean resp = false;

        if(selecao==0){
            if (pivo.getHouse().compareTo(temp.elemento.getHouse()) > 0) resp = true;
            else if (pivo.getHouse().compareTo(temp.elemento.getHouse()) < 0) resp = false;
            else {
                if (pivo.getName().compareTo(temp.elemento.getName()) > 0) resp = true;
                else resp = false;
                comparacoes++;
            }
            comparacoes += 2;
    ...
    static Personagem mediaPersonagem(CtrlPersonagem esq, CtrlPersonagem dir) {
        int x=0;
        Personagem resp;
    ...
    static void quickSort(CtrlPersonagem esq, CtrlPersonagem dir) {
        CtrlPersonagem i=esq, j=dir;
        Personagem pivo = mediaPersonagem(esq, dir);
    ...
    static void log(long tempoDeExecucao) throws Exception{
        FileWriter file = new FileWriter ("802512_quicksort2.txt");
