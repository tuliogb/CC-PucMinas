...
/**
 * Questao 9 - Trabalho Pratico 2 - Aeds II - CC_PUCMINAS
 ...
 *  
 *  compereHairColour: Compara de acordo com o atributo hairColour e desempata por nome.
...
public class CtrlPersonagem{
    ...
    static boolean compereHairColour(Personagem x, Personagem y){
        boolean resp = false;

        if (x.getHairColour().compareTo(y.getHairColour()) > 0) resp = true;
        else if (x.getHairColour().compareTo(y.getHairColour()) < 0) resp = false;
        else{
            if (x.getName().compareTo(y.getName()) > 0) resp = true;
            else resp = false;
            comparacoes++;
        }
        comparacoes+=2;
    ...
    static void heapSort() {
        Personagem[] tmp = new Personagem[entradas+1];
        for(int i = 0; i < entradas; i++){
            tmp[i+1] = Lista[i];
        }
        Lista = tmp;
    ...
    static void construir(int tamHeap){
        for(int i = tamHeap; i > 1 && compereHairColour(Lista[i],Lista[i/2]); i /= 2){
            swap(i, i/2);
        }
    }
    ...
    static void reconstruir(int tamHeap){
        ...
        while(i <= (tamHeap/2)){
            ...
            if(!compereHairColour(Lista[i],Lista[filho])){
                swap(i, filho);
                i = filho;
            }else{
                i = tamHeap;
    ...
    static int getMaiorFilho(int i, int tamHeap){
        int filho;
        if (2*i == tamHeap || compereHairColour(Lista[2*i],Lista[2*i+1])){
            filho = 2*i;
        } else {
            filho = 2*i + 1;
        }
    ...
    static void log(long tempoDeExecucao) throws Exception{
        FileWriter file = new FileWriter ("802512_heapsort.txt");
