...
/**
 * Questao 13 - Trabalho Pratico 2 - Aeds II - CC_PUCMINAS
 ...
 *  compereActor: Compara de acordo com o atributo actorName e desempata por nome.
 *  mergeSort: Metodo de ordenacao.
...
public class CtrlPersonagem{
    ...
    static void mostrarLista(){
        for (int i=0; i<entradas; i++){
            ...
            System.out.println(Lista[i].getId() +"   "+ Lista[i].getName() +"   "+ Lista[i].getActorName());
        }
    }


    ///////////////////////////////////////////////////////////////////// EXERCICIO >> Obs: O codigo esta certo, esta ordenando por actorname e desempatando por nome
    ...
    static boolean compereActor(Personagem x, Personagem y){
        boolean resp = true;   // x<=y true

        if (x.getActorName().compareTo(y.getActorName()) > 0) resp = false;

        else if (x.getActorName().compareTo(y.getActorName()) < 0) resp = true;
        else{
            if (x.getName().compareTo(y.getName()) > 0) resp = false;
            else resp = true;
            comparacoes++;
        }
    ...
    static void intercalar(int esq, int meio, int dir) {
        ...
        while (i < n1 && j < n2) {
            if (compereActor(a1[i], a2[j])) Lista[k++] = a1[i++];
            else Lista[k++] = a2[j++];
            movimentacoes++;            
        }

        while (i < n1) Lista[k++] = a1[i++];
        while (j < n2) Lista[k++] = a2[j++];
        
    }

    static void log(long tempoDeExecucao) throws Exception{
        FileWriter file = new FileWriter ("802512_mergesort.txt");
