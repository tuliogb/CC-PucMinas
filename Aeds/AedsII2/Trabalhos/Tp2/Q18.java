...
/**
 * Questao 18 - Trabalho Pratico 2 - Aeds II - CC_PUCMINAS
 ...
 *  quickSort: Metodo de ordenacao com k=10.
 */
...
public class CtrlPersonagem{
    ...
    static int tamBase=0, entradas=0, comparacoes=0, movimentacoes=0, maxTam=405, k=10;
    ...
    static boolean compareHouse(Personagem pivo, Personagem temp, int selecao){
        boolean resp = false; 

        if(selecao == 0){
            if(pivo.getHouse().compareTo(temp.getHouse())>0) resp = true;
            else if(pivo.getHouse().compareTo(temp.getHouse())<0) resp = false;
            else{
                if(pivo.getName().compareTo(temp.getName())>0) resp = true;
                else resp = false;
                comparacoes++;
            }
            comparacoes+=2;
    ...
    static void log(long tempoDeExecucao) throws Exception{
        FileWriter file = new FileWriter ("802512_qucksortP.txt");
