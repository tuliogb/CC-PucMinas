...
/**
 * Questao 8 - Trabalho Pratico 2 - Aeds II - CC_PUCMINAS
 ...
 *  compareEyeColour: Compara os termos de acordo com o atributo eyecolour e desempate name.
 *  shellSort: metodo de ordenacao.
 *  insertionShell: Insercao modificada para o shellSort.
*/
...
int compareEyeColour(Personagem j, Personagem temp){
    int resp = 0; 

    if(strcmp(j.eyeColour, temp.eyeColour)>0) resp=1;
    else if(strcmp(j.eyeColour, temp.eyeColour)<0) resp=-1;
    else{
        comparacoes++;
        if(strcmp(j.name, temp.name)>0) resp=1;
        else resp=-1;
    }
    comparacoes+=2;
    return resp;
...
void insertionShell(int cor, int h){
    for(int i=h+cor; i<entradas; i+=h){
        Personagem temp = Lista[i];
        int j=i-h;

        while(j>=0 && compareEyeColour(Lista[j], temp)>0){
            Lista[j+h]=Lista[j];
            movimentacoes++; j=j-h;
        }
        Lista[j+h]=temp;  movimentacoes++;
    }
}

void shellSort(){
    int h=1;
    do{ h=(h*3)+1; }while (h<entradas);
...
void Arqlog(){
    ...
    FILE *file = fopen("802512_shellsort.txt", "w");
