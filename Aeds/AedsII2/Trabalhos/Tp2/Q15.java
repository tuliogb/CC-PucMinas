...
/**
 * Questao 15 - Trabalho Pratico 2 - Aeds II - CC_PUCMINAS
 ...
 *  selectionSort: Metodo de ordenacao por selecao com k=10
 */
...
public class CtrlPersonagem{
    ...
    static void selectionSort(){
        for(int i=0; i<10; i++){
            ...
            for(int j=i+1; j<entradas; j++){
                comparacoes++;
                if(Lista[menor].getName().compareTo(Lista[j].getName()) > 0) menor=j;
            }
            swap(i,menor);
        }
    }
    ...
    static void log(long tempoDeExecucao) throws Exception{
        FileWriter file = new FileWriter ("802512_selecaoParcial.txt");
