...
/**
 * Questao 12 - Trabalho Pratico 2 - Aeds II - CC_PUCMINAS
...
bool compareHairColour(Personagem j, Personagem j1){
    bool resp = false; 

    if(strcmp(j.hairColour, j1.hairColour) > 0) resp = true;
    else if(strcmp(j.hairColour, j1.hairColour) < 0) resp = false;
    else{
        if(strcmp(j.name, j1.name) > 0) resp = true;
        else resp = false;
        comparacoes++;
    }
    comparacoes+=2;
...
void bolha(){
    for(int i=entradas-1; i>0; i--){
        for(int j=0; j<i; j++){
            if(compareHairColour(Lista[j], Lista[j+1])) swap(j, j+1);
        }
    }
}
...
void Arqlog(){
    ...
    FILE *file = fopen("802512_bolha.txt", "w");
