...
/**
 * Questao 4 - Trabalho Pratico 4 - Aeds II - CC_PUCMINAS
...
class NoAN {
    ...
    NoAN(Personagem elemento) {
        this(elemento, false);
    }

    NoAN(Personagem elemento, boolean cor) {
...
public class CtrlPersonagem {
    ...
    static NoAN inserir(Personagem elemento) {
        if (raiz == null) {
    ...
    static void inserir(Personagem elemento, NoAN bisavo, NoAN avo, NoAN pai, NoAN i) {
        if (i == null) {
            if (elemento.getName().compareTo(pai.elemento.getName()) < 0) i = pai.esq = new NoAN(elemento, true);
            else i = pai.dir = new NoAN(elemento, true);
            comparacoes++;
            ...
            if (elemento.getName().compareTo(i.elemento.getName()) < 0) inserir(elemento, avo, pai, i, i.esq);
            else if (elemento.getName().compareTo(i.elemento.getName()) > 0) inserir(elemento, avo, pai, i, i.dir);
            comparacoes+=2;
        }
    }

    static void balancear(NoAN bisavo, NoAN avo, NoAN pai, NoAN i) {
        if (pai.cor == true) {
            if (pai.elemento.getName().compareTo(avo.elemento.getName()) > 0) { 
                if (i.elemento.getName().compareTo(pai.elemento.getName()) > 0) avo = rotacaoEsq(avo);
                else avo = rotacaoDirEsq(avo);
                comparacoes+=2;

            } else {
                if (i.elemento.getName().compareTo(pai.elemento.getName()) < 0) avo = rotacaoDir(avo);
                else avo = rotacaoEsqDir(avo);
                comparacoes++;
            }

            if (bisavo == null) raiz = avo;
            else if (avo.elemento.getName().compareTo(bisavo.elemento.getName()) < 0) bisavo.esq = avo;
            else bisavo.dir = avo;
            comparacoes++;
    ...
    static NoAN rotacaoDir(NoAN no) {
        NoAN noEsq = no.esq;
    ...
    static NoAN rotacaoEsq(NoAN no) {
        NoAN noDir = no.dir;
    ...
    static NoAN rotacaoDirEsq(NoAN no) {
        no.dir = rotacaoDir(no.dir);
        return rotacaoEsq(no);
    }

    static NoAN rotacaoEsqDir(NoAN no) {
    ...
    static boolean pesquisar(NoAN i, String chave){
        boolean resp = false;
    ...
    static void log(long tempoDeExecucao) throws Exception {
        FileWriter file = new FileWriter("802512_alvinegra.txt");
