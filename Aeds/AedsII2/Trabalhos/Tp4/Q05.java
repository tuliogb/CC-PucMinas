 ...
 *  setaEntrada: Equanto a entrada for diferente de "FIM", chama-se o medoto setaHash(passando a entrada lida).
*/
...
public class CtrlPersonagem{
    ...
    static int tamBase=0, tamHash=21, tamReserva=9, reserva=0, maxTam=405;
    static Personagem[] Base =  new Personagem[maxTam];
    static Personagem[] Hash = new Personagem[30];
    ...
    static void setaHash(String id){
    ...
    static void inserirHash(Personagem elemento){
        int posicao = posicao(elemento.getName());
        if(Hash[posicao]==null) Hash[posicao] = elemento;
        else if(reserva<tamReserva){
            Hash[tamHash+reserva] = elemento;
            reserva++;
        }
    }
    ...
    static void pesquisar(){
        ...
        while(!input.equals("FIM")){
            ...
            System.out.println(pesquisar(input) ? " SIM" : " NAO");
            input = sc.nextLine();
        } 
    }

    static boolean pesquisar(String nome){
        boolean resp = false;
        int p = posicao(nome);
        ...
        if(Hash[p] != null){
            if(Hash[p].getName().compareTo(nome)==0){ 
                resp = true;  
                System.out.print(" (Posicao: " +p+ ")");
            }
            else{
                for (int i=0; i<reserva; i++) {
                    if (Hash[tamHash+i].getName().compareTo(nome)==0){
                        resp = true;
                        System.out.print(" (Posicao: " +(tamHash+i)+ ")");
                        i = reserva;
                    }
                }
            }
