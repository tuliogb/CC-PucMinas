...
class Jogador {
     ...
     * 
     * Exercicio: Metodo de ordenacao por selecao -->> OrgSelecao();
    */
	...
	public static void ArqLog(long tempo) {
		String nomeArq = "matrícula_selecao.txt";
		String mtr = "1441272";
    ...
    public static void OrgSelecao(ArrayList<Jogador> lista){
    
        for(int i=0;i<(lista.size()-1);i++){
            int menor = i;
            for(int j=i+1;j<lista.size();j++){
                comp++;
                if(lista.get(menor).getNome().compareTo(lista.get(j).getNome())>0) menor=j;
            }
            Collections.swap(lista,menor,i);
            comp+=3;
        }
    }
