...
class Jogador {
     ...
     * 
     * Exercicio: Metodo de ordenacao quickSort parcial(10) -->> atributo Estado de nascimento;
    */
	...
	public static void ArqLog(long tempo) {
		String nomeArq = "matrícula_quicksortp.txt";
		String mtr = "1441272";
	...
	public static void ParcialQuick(ArrayList<Jogador> lista, int k, int esq, int dir) {
		int i = esq, j = dir;
		Jogador pivo = lista.get((dir + esq)/2);
		while (i <= j) {
			while (lista.get(i).getEstadoNascimento().compareTo(pivo.getEstadoNascimento()) < 0 || (lista.get(i).getEstadoNascimento().equals(pivo.getEstadoNascimento()) && lista.get(i).getNome().compareTo(pivo.getNome()) < 0)){
				i++;
			}
			while (lista.get(j).getEstadoNascimento().compareTo(pivo.getEstadoNascimento()) > 0 || (lista.get(j).getEstadoNascimento().equals(pivo.getEstadoNascimento()) && lista.get(j).getNome().compareTo(pivo.getNome()) > 0)){
				j--;
			}
			if (i <= j) {
		...
		if (esq<j) {
			ParcialQuick(lista, k, esq, j);
		}
		if (i<dir && i<k){
			ParcialQuick(lista, k, i, dir);
		}
	}
    ...
    public static void main(String[] args) {
        ...
        ParcialQuick(lista,10,0,lista.size()-1);
        MostraLista(lista);
