...
class Jogador {
     ...
     * 
     * Exercicio: Metodo de ordenacao por selecaoParcial -->> So ira retorna os 10 menores elementos(at:nome);
    */
	...
	public static void ArqLog(long tempo) {
		String nomeArq = "matrícula_selecaoptxt";
		String mtr = "1441272";
	...
	public static ArrayList<Jogador> selecaoParcial(ArrayList<Jogador> lista) {
		int tam = lista.size();
		int k = 10;
		ArrayList<Jogador> parcial = new ArrayList<>();

		for (int i=0;i<k;i++) {
			int menor=i;
			for (int j=(i+1);j<tam;j++) {
				if (lista.get(j).getNome().compareTo(lista.get(menor).getNome())<0) {
					menor = j;
				}
			}
    ...
    public static void main(String[] args) {
        ...
        MostraLista(selecaoParcial(lista));
