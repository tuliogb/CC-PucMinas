...
class Jogador {
     ...
     * 
     * Exercicio: Metodo de ordenacao por insercao -->> OrgInsercao();
    */
    ...
    public static void OrgInsercao(ArrayList<Jogador> lista) {
        for (int i = 1; i < lista.size(); i++) {
            Jogador tmp = lista.get(i);
            int j = i - 1;
            while (j >= 0 && (lista.get(j).getAnoNascimento() > tmp.getAnoNascimento() || (lista.get(j).getAnoNascimento() == tmp.getAnoNascimento() &&  lista.get(j).getNome().compareTo(tmp.getNome()) > 0))) {
